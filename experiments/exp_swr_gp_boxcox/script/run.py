#!/usr/bin/env python3
"""SWR-GP (Box-Cox λ=0.2) application study experiment — self-contained.

Fits GPSWR (Gaussian kernel, NNGP covariance) on Box-Cox-transformed Big Sur
streamflow.  K ∈ {1,2,3,4}; model selected by BIC(GLS).  Back-transformation
uses the delta-method approximation to the conditional mean.
...
Usage (from repository root):
    .venv/bin/python experiments/exp_swr_gp_boxcox/script/run.py
"""
...
def _inverse_boxcox_mean_delta(
    ...
    """Delta-method back-transform: approximate E[g^{-1}(Z)] for Z~N(mu_t, sigma_t^2)."""
...
def main() -> int:
    ...
    for K in K_VALUES:
        ...
        model = GPSWR(
            ...
            max_lag=MAX_LAG,
            maxiter=GP_MAXITER,
            log_transform=False,   # transform handled externally
            seed=SEED,
            verbose=False,
        ...
        krig_train_t = krig_full_t[:n_train]
        krig_test_t  = krig_full_t[n_train:]
        theta        = np.asarray(model.theta_, dtype=float)

        # Per-observation kriging sigma on transformed scale
        _, F_full = build_nngp_matrices(len(full_y_t), M, theta)
        sigma_krig_train_t = np.sqrt(np.maximum(F_full[:n_train], 1e-12))
        sigma_krig_test_t  = np.sqrt(np.maximum(F_full[n_train:], 1e-12))
        ...
        krig_train = _inverse_boxcox_mean_delta(krig_train_t, sigma_krig_train_t, lam, shift)
        krig_test  = _inverse_boxcox_mean_delta(krig_test_t,  sigma_krig_test_t,  lam, shift)
        ...
        # CRPS — Monte Carlo on transformed scale, back-transformed
        _crps_kw_sim = dict(
        ...
        metrics["krig_train"]["CRPS"] = float(compute_crps(
            ...
            family="gaussian", sigma=sigma_krig_train_t,
            response_transform="boxcox", response_shift=shift, response_lambda=lam))
        metrics["krig_test"]["CRPS"] = float(compute_crps(
            y_test, krig_test, y_transformed=y_test_t, mu_transformed=krig_test_t,
            family="gaussian", sigma=sigma_krig_test_t,
            response_transform="boxcox", response_shift=shift, response_lambda=lam))

        # BIC(GLS) for within-model selection (on transformed scale)
        B, F = build_nngp_matrices(n_train, M, theta)
        ll      = gls_log_likelihood(y_train_t, sim_train_t, B, F, M)
        ...
        if bic_val < best_bic:
            ...
            best = dict(
                ...
                sigma_krig_train_t=sigma_krig_train_t, sigma_krig_test_t=sigma_krig_test_t,
    ...
    innovations = {
        ...
        "krig_test": compute_standardized_innovations(
            y_test_t, best["krig_test_t"], best["sigma_krig_test_t"]),
    }

    # Plots
    _plot_qq(innovations["sim_test"]["z"],
             out_dir / "qq_sim_test.png",  "Innovation QQ: SWR-GP-BC sim (test)")
    _plot_qq(innovations["krig_test"]["z"],
             out_dir / "qq_krig_test.png", "Innovation QQ: SWR-GP-BC krig (test)")
    _plot_pit(calibration["sim_test"]["pit"],
              out_dir / "pit_sim_test.png",  "PIT: SWR-GP-BC sim (test)")
    _plot_pit(calibration["krig_test"]["pit"],
              out_dir / "pit_krig_test.png", "PIT: SWR-GP-BC krig (test)")
