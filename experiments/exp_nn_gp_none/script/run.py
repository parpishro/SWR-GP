#!/usr/bin/env python3
"""NN-GP (no transform) application study experiment — self-contained.

Fits GPNN (two-layer MLP mean + NNGP covariance, GLS iterations) on Big Sur
streamflow.  Architecture is fixed (hidden_dim=24, dropout=0.0,
weight_decay=1e-3); model selection uses a time-ordered held-out CRPS on
the tail of the training period.
...
def _setup_logger(log_file: Path) -> logging.Logger:
    logger = logging.getLogger("exp_nn_gp_none")
...
def main() -> int:
    ...
    sigma_sel     = float(np.sqrt(max(sel_theta[0] + sel_theta[3], 1e-12)))
    krig_subval   = selector.forecast(rain_train, y_train)
    krig_val      = np.maximum(krig_subval[sub_end:], 0.0)
    selection_score = float(compute_crps(y_val, krig_val, family="gaussian", sigma=sigma_sel))
    ...
    metrics["sim_train"]["CRPS"]  = float(compute_crps(y_train, sim_train_arr,  family="gaussian", sigma=sigma_marginal))
    metrics["sim_test"]["CRPS"]   = float(compute_crps(y_test,  sim_test_arr,   family="gaussian", sigma=sigma_marginal))
    ...
    _plot_qq(innovations["sim_test"]["z"],
             out_dir / "qq_sim_test.png",  "Innovation QQ: NN-GP sim (test)")
    _plot_qq(innovations["krig_test"]["z"],
             out_dir / "qq_krig_test.png", "Innovation QQ: NN-GP krig (test)")
    _plot_pit(calibration["sim_test"]["pit"],
              out_dir / "pit_sim_test.png",  "NN-GP, simulation")
    _plot_pit(calibration["krig_test"]["pit"],
              out_dir / "pit_krig_test.png", "NN-GP, corrected")
    ...
    payload: dict[str, Any] = {
        ...
        "config": {
            ...
            "selection": "tail-of-training heldout CRPS (kriging)",
        },
        "selection": {
            "criterion": "val_CRPS_krig(train_heldout)",
